...
class measurement_delta_position(object):
    """
    ...
    __slots__ = ["icd_measurement_delta_position", "header", "time_of_validity", "reference_frame", "delta_t", "num_meas", "term1", "term2", "term3", "covariance", "error_model", "num_error_model_params", "error_model_params", "num_integrity", "integrity"]

    __typenames__ = ["int8_t", "aspn23_lcm.type_header", "aspn23_lcm.type_timestamp", "int8_t", "double", "int16_t", "double", "double", "double", "double", "int8_t", "int32_t", "double", "int16_t", "aspn23_lcm.type_integrity"]

    __dimensions__ = [None, None, None, None, None, None, None, None, None, ["num_meas", "num_meas"], None, None, ["num_error_model_params"], None, ["num_integrity"]]

    REFERENCE_FRAME_NED = 0
    """
    ...
    """
    REFERENCE_FRAME_SENSOR_START = 1
    """
    ...
    def __init__(self):
        self.icd_measurement_delta_position = 0
        """
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(measurement_delta_position._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack(">b", self.icd_measurement_delta_position))
        ...
        buf.write(struct.pack(">bdhddd", self.reference_frame, self.delta_t, self.num_meas, self.term1, self.term2, self.term3))
    ...
    def decode(data: bytes):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != measurement_delta_position._get_packed_fingerprint():
            raise ValueError("Decode error")
        return measurement_delta_position._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        self = measurement_delta_position()
        self.icd_measurement_delta_position = struct.unpack(">b", buf.read(1))[0]
        self.header = aspn23_lcm.type_header._decode_one(buf)
        self.time_of_validity = aspn23_lcm.type_timestamp._decode_one(buf)
        self.reference_frame, self.delta_t, self.num_meas, self.term1, self.term2, self.term3 = struct.unpack(">bdhddd", buf.read(35))
        self.covariance = []
        for i0 in range(self.num_meas):
    ...
    def _get_hash_recursive(parents):
        if measurement_delta_position in parents: return 0
        newparents = parents + [measurement_delta_position]
        tmphash = (0xe74dc0282d04d697+ aspn23_lcm.type_header._get_hash_recursive(newparents)+ aspn23_lcm.type_timestamp._get_hash_recursive(newparents)+ aspn23_lcm.type_integrity._get_hash_recursive(newparents)) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        if measurement_delta_position._packed_fingerprint is None:
            measurement_delta_position._packed_fingerprint = struct.pack(">Q", measurement_delta_position._get_hash_recursive([]))
        return measurement_delta_position._packed_fingerprint

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", measurement_delta_position._get_packed_fingerprint())[0]
