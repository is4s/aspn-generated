...
class measurement_temperature(object):
    """ Ambient temperature as measured by a sensor """

    __slots__ = ["icd_measurement_temperature", "header", "time_of_validity", "temperature", "variance", "error_model", "num_error_model_params", "error_model_params", "num_integrity", "integrity"]
    ...
    def __init__(self):
        self.icd_measurement_temperature = 0
        """
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(measurement_temperature._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack(">b", self.icd_measurement_temperature))
        ...
        buf.write(struct.pack(">ddbi", self.temperature, self.variance, self.error_model, self.num_error_model_params))
    ...
    def decode(data: bytes):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != measurement_temperature._get_packed_fingerprint():
            raise ValueError("Decode error")
        return measurement_temperature._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        self = measurement_temperature()
        self.icd_measurement_temperature = struct.unpack(">b", buf.read(1))[0]
        self.header = aspn23_lcm.type_header._decode_one(buf)
        self.time_of_validity = aspn23_lcm.type_timestamp._decode_one(buf)
        self.temperature, self.variance, self.error_model, self.num_error_model_params = struct.unpack(">ddbi", buf.read(21))
    ...
    def _get_hash_recursive(parents):
        if measurement_temperature in parents: return 0
        newparents = parents + [measurement_temperature]
        tmphash = (0xa1b01f778c459dc9+ aspn23_lcm.type_header._get_hash_recursive(newparents)+ aspn23_lcm.type_timestamp._get_hash_recursive(newparents)+ aspn23_lcm.type_integrity._get_hash_recursive(newparents)) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        if measurement_temperature._packed_fingerprint is None:
            measurement_temperature._packed_fingerprint = struct.pack(">Q", measurement_temperature._get_hash_recursive([]))
        return measurement_temperature._packed_fingerprint

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", measurement_temperature._get_packed_fingerprint())[0]
