...
class measurement_direction_of_motion_2d(object):
    """ Direction of motion represented in a two-dimensional reference frame as defined by the enum. """

    __slots__ = ["icd_measurement_direction_of_motion_2d", "header", "time_of_validity", "reference", "obs", "variance", "error_model", "num_error_model_params", "error_model_params", "num_integrity", "integrity"]
    ...
    def __init__(self):
        self.icd_measurement_direction_of_motion_2d = 0
        """
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(measurement_direction_of_motion_2d._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack(">b", self.icd_measurement_direction_of_motion_2d))
        ...
        buf.write(struct.pack(">bddbi", self.reference, self.obs, self.variance, self.error_model, self.num_error_model_params))
    ...
    def decode(data: bytes):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != measurement_direction_of_motion_2d._get_packed_fingerprint():
            raise ValueError("Decode error")
        return measurement_direction_of_motion_2d._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        self = measurement_direction_of_motion_2d()
        self.icd_measurement_direction_of_motion_2d = struct.unpack(">b", buf.read(1))[0]
        self.header = aspn23_lcm.type_header._decode_one(buf)
        self.time_of_validity = aspn23_lcm.type_timestamp._decode_one(buf)
        self.reference, self.obs, self.variance, self.error_model, self.num_error_model_params = struct.unpack(">bddbi", buf.read(22))
    ...
    def _get_hash_recursive(parents):
        if measurement_direction_of_motion_2d in parents: return 0
        newparents = parents + [measurement_direction_of_motion_2d]
        tmphash = (0x929354a02d11564c+ aspn23_lcm.type_header._get_hash_recursive(newparents)+ aspn23_lcm.type_timestamp._get_hash_recursive(newparents)+ aspn23_lcm.type_integrity._get_hash_recursive(newparents)) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        if measurement_direction_of_motion_2d._packed_fingerprint is None:
            measurement_direction_of_motion_2d._packed_fingerprint = struct.pack(">Q", measurement_direction_of_motion_2d._get_hash_recursive([]))
        return measurement_direction_of_motion_2d._packed_fingerprint

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", measurement_direction_of_motion_2d._get_packed_fingerprint())[0]
