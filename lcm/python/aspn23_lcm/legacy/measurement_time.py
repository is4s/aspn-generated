...
class measurement_time(object):
    ...
    __slots__ = ["icd_measurement_time", "header", "time_of_validity", "time_of_validity_attosec", "num_obs", "clock_id", "elapsed_nsec", "elapsed_attosec", "digits_of_precision", "covariance", "error_model", "num_error_model_params", "error_model_params", "num_integrity", "integrity"]
    ...
    __dimensions__ = [None, None, None, None, None, ["num_obs"], ["num_obs"], ["num_obs"], None, ["num_obs", "num_obs"], None, None, ["num_error_model_params"], None, ["num_integrity"]]
    ...
    def _encode_one(self, buf):
        ...
        buf.write(struct.pack('>%dq' % self.num_obs, *self.elapsed_nsec[:self.num_obs]))
        buf.write(struct.pack('>%di' % self.num_obs, *self.elapsed_attosec[:self.num_obs]))
        buf.write(struct.pack(">h", self.digits_of_precision))
        for i0 in range(self.num_obs):
            buf.write(struct.pack('>%dd' % self.num_obs, *self.covariance[i0][:self.num_obs]))
    ...
    def _decode_one(buf):
        ...
        self.elapsed_nsec = struct.unpack('>%dq' % self.num_obs, buf.read(self.num_obs * 8))
        self.elapsed_attosec = struct.unpack('>%di' % self.num_obs, buf.read(self.num_obs * 4))
        self.digits_of_precision = struct.unpack(">h", buf.read(2))[0]
        self.covariance = []
        for i0 in range(self.num_obs):
            self.covariance.append(struct.unpack('>%dd' % self.num_obs, buf.read(self.num_obs * 8)))
    ...
    def _get_hash_recursive(parents):
        if measurement_time in parents: return 0
        newparents = parents + [measurement_time]
        tmphash = (0xacc221e035ba92c7+ aspn23_lcm.type_header._get_hash_recursive(newparents)+ aspn23_lcm.type_timestamp._get_hash_recursive(newparents)+ aspn23_lcm.type_integrity._get_hash_recursive(newparents)) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        if measurement_time._packed_fingerprint is None:
            measurement_time._packed_fingerprint = struct.pack(">Q", measurement_time._get_hash_recursive([]))
