...
class metadata_magnetic_field(object):
    ...
    __slots__ = ["icd_metadata_magnetic_field", "info", "time_of_validity", "mounting", "num_meas", "k", "b"]

    __typenames__ = ["int8_t", "aspn23_lcm.type_metadataheader", "aspn23_lcm.type_timestamp", "aspn23_lcm.type_mounting", "int16_t", "double", "double"]

    __dimensions__ = [None, None, None, None, None, ["num_meas", "num_meas"], ["num_meas"]]
    ...
    def _encode_one(self, buf):
        ...
        assert self.mounting._get_packed_fingerprint() == aspn23_lcm.type_mounting._get_packed_fingerprint()
    ...
    def _decode_one(buf):
        ...
        self.k = []
        for i0 in range(self.num_meas):
            self.k.append(struct.unpack('>%dd' % self.num_meas, buf.read(self.num_meas * 8)))
        self.b = struct.unpack('>%dd' % self.num_meas, buf.read(self.num_meas * 8))
        return self

    @staticmethod
    def _get_hash_recursive(parents):
        if metadata_magnetic_field in parents: return 0
        newparents = parents + [metadata_magnetic_field]
        tmphash = (0xec9dbdee1877dc68+ aspn23_lcm.type_metadataheader._get_hash_recursive(newparents)+ aspn23_lcm.type_timestamp._get_hash_recursive(newparents)+ aspn23_lcm.type_mounting._get_hash_recursive(newparents)) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        if metadata_magnetic_field._packed_fingerprint is None:
            metadata_magnetic_field._packed_fingerprint = struct.pack(">Q", metadata_magnetic_field._get_hash_recursive([]))
