...
class measurement_velocity(object):
    """
    ...
    __slots__ = ["icd_measurement_velocity", "header", "time_of_validity", "reference_frame", "x", "y", "z", "num_meas", "covariance", "error_model", "num_error_model_params", "error_model_params", "num_integrity", "integrity"]

    __typenames__ = ["int8_t", "aspn23_lcm.type_header", "aspn23_lcm.type_timestamp", "int8_t", "double", "double", "double", "int16_t", "double", "int8_t", "int32_t", "double", "int16_t", "aspn23_lcm.type_integrity"]

    __dimensions__ = [None, None, None, None, None, None, None, None, ["num_meas", "num_meas"], None, None, ["num_error_model_params"], None, ["num_integrity"]]
    ...
    def __init__(self):
        self.icd_measurement_velocity = 0
        """
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(measurement_velocity._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack(">b", self.icd_measurement_velocity))
        ...
        buf.write(struct.pack(">bdddh", self.reference_frame, self.x, self.y, self.z, self.num_meas))
    ...
    def decode(data: bytes):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != measurement_velocity._get_packed_fingerprint():
            raise ValueError("Decode error")
        return measurement_velocity._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        self = measurement_velocity()
        self.icd_measurement_velocity = struct.unpack(">b", buf.read(1))[0]
        self.header = aspn23_lcm.type_header._decode_one(buf)
        self.time_of_validity = aspn23_lcm.type_timestamp._decode_one(buf)
        self.reference_frame, self.x, self.y, self.z, self.num_meas = struct.unpack(">bdddh", buf.read(27))
        self.covariance = []
        for i0 in range(self.num_meas):
    ...
    def _get_hash_recursive(parents):
        if measurement_velocity in parents: return 0
        newparents = parents + [measurement_velocity]
        tmphash = (0x3f73efd4cc373c19+ aspn23_lcm.type_header._get_hash_recursive(newparents)+ aspn23_lcm.type_timestamp._get_hash_recursive(newparents)+ aspn23_lcm.type_integrity._get_hash_recursive(newparents)) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        if measurement_velocity._packed_fingerprint is None:
            measurement_velocity._packed_fingerprint = struct.pack(">Q", measurement_velocity._get_hash_recursive([]))
        return measurement_velocity._packed_fingerprint

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", measurement_velocity._get_packed_fingerprint())[0]
