...
class measurement_position_velocity_attitude(object):
    """
    ...
    __slots__ = ["icd_measurement_position_velocity_attitude", "header", "time_of_validity", "reference_frame", "p1", "p2", "p3", "v1", "v2", "v3", "quaternion", "num_meas", "covariance", "error_model", "num_error_model_params", "error_model_params", "num_integrity", "integrity"]

    __typenames__ = ["int8_t", "aspn23_lcm.type_header", "aspn23_lcm.type_timestamp", "int8_t", "double", "double", "double", "double", "double", "double", "double", "int16_t", "double", "int8_t", "int32_t", "double", "int16_t", "aspn23_lcm.type_integrity"]

    __dimensions__ = [None, None, None, None, None, None, None, None, None, None, [4], None, ["num_meas", "num_meas"], None, None, ["num_error_model_params"], None, ["num_integrity"]]
    ...
    def __init__(self):
        self.icd_measurement_position_velocity_attitude = 0
        """
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(measurement_position_velocity_attitude._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack(">b", self.icd_measurement_position_velocity_attitude))
        ...
        buf.write(struct.pack(">bdddddd", self.reference_frame, self.p1, self.p2, self.p3, self.v1, self.v2, self.v3))
        buf.write(struct.pack('>4d', *self.quaternion[:4]))
        buf.write(struct.pack(">h", self.num_meas))
        for i0 in range(self.num_meas):
            buf.write(struct.pack('>%dd' % self.num_meas, *self.covariance[i0][:self.num_meas]))
    ...
    def decode(data: bytes):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != measurement_position_velocity_attitude._get_packed_fingerprint():
            raise ValueError("Decode error")
        return measurement_position_velocity_attitude._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        self = measurement_position_velocity_attitude()
        self.icd_measurement_position_velocity_attitude = struct.unpack(">b", buf.read(1))[0]
        self.header = aspn23_lcm.type_header._decode_one(buf)
        self.time_of_validity = aspn23_lcm.type_timestamp._decode_one(buf)
        self.reference_frame, self.p1, self.p2, self.p3, self.v1, self.v2, self.v3 = struct.unpack(">bdddddd", buf.read(49))
        self.quaternion = struct.unpack('>4d', buf.read(32))
        self.num_meas = struct.unpack(">h", buf.read(2))[0]
        self.covariance = []
        for i0 in range(self.num_meas):
            self.covariance.append(struct.unpack('>%dd' % self.num_meas, buf.read(self.num_meas * 8)))
    ...
    def _get_hash_recursive(parents):
        if measurement_position_velocity_attitude in parents: return 0
        newparents = parents + [measurement_position_velocity_attitude]
        tmphash = (0xf94f209bf89d3e2e+ aspn23_lcm.type_header._get_hash_recursive(newparents)+ aspn23_lcm.type_timestamp._get_hash_recursive(newparents)+ aspn23_lcm.type_integrity._get_hash_recursive(newparents)) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        if measurement_position_velocity_attitude._packed_fingerprint is None:
            measurement_position_velocity_attitude._packed_fingerprint = struct.pack(">Q", measurement_position_velocity_attitude._get_hash_recursive([]))
        return measurement_position_velocity_attitude._packed_fingerprint

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", measurement_position_velocity_attitude._get_packed_fingerprint())[0]
