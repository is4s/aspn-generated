...
class measurement_altitude(object):
    """ Height above a user-specified reference frame. """

    __slots__ = ["icd_measurement_altitude", "header", "time_of_validity", "reference", "altitude", "variance", "error_model", "num_error_model_params", "error_model_params", "num_integrity", "integrity"]
    ...
    def __init__(self):
        self.icd_measurement_altitude = 0
        """
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(measurement_altitude._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack(">b", self.icd_measurement_altitude))
        ...
        buf.write(struct.pack(">bddbi", self.reference, self.altitude, self.variance, self.error_model, self.num_error_model_params))
    ...
    def decode(data: bytes):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != measurement_altitude._get_packed_fingerprint():
            raise ValueError("Decode error")
        return measurement_altitude._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        self = measurement_altitude()
        self.icd_measurement_altitude = struct.unpack(">b", buf.read(1))[0]
        self.header = aspn23_lcm.type_header._decode_one(buf)
        self.time_of_validity = aspn23_lcm.type_timestamp._decode_one(buf)
        self.reference, self.altitude, self.variance, self.error_model, self.num_error_model_params = struct.unpack(">bddbi", buf.read(22))
    ...
    def _get_hash_recursive(parents):
        if measurement_altitude in parents: return 0
        newparents = parents + [measurement_altitude]
        tmphash = (0x5b7a67ef03e44e6b+ aspn23_lcm.type_header._get_hash_recursive(newparents)+ aspn23_lcm.type_timestamp._get_hash_recursive(newparents)+ aspn23_lcm.type_integrity._get_hash_recursive(newparents)) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        if measurement_altitude._packed_fingerprint is None:
            measurement_altitude._packed_fingerprint = struct.pack(">Q", measurement_altitude._get_hash_recursive([]))
        return measurement_altitude._packed_fingerprint

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", measurement_altitude._get_packed_fingerprint())[0]
