...
class metadata_BeiDou_ephemeris(object):
    ...
    __slots__ = ["icd_metadata_BeiDou_ephemeris", "info", "time_of_validity", "prn", "clock", "orbit", "t_gd1", "t_gd2", "aodc", "aode"]

    __typenames__ = ["int8_t", "aspn23_lcm.type_metadataheader", "aspn23_lcm.type_timestamp", "int32_t", "aspn23_lcm.type_satnav_clock", "aspn23_lcm.type_kepler_orbit", "double", "double", "int16_t", "int16_t"]
    ...
    def _decode_one(buf):
        ...
        self.t_gd1, self.t_gd2, self.aodc, self.aode = struct.unpack(">ddhh", buf.read(20))
        return self

    @staticmethod
    def _get_hash_recursive(parents):
        if metadata_BeiDou_ephemeris in parents: return 0
        newparents = parents + [metadata_BeiDou_ephemeris]
        tmphash = (0xd9732d27913fec47+ aspn23_lcm.type_metadataheader._get_hash_recursive(newparents)+ aspn23_lcm.type_timestamp._get_hash_recursive(newparents)+ aspn23_lcm.type_satnav_clock._get_hash_recursive(newparents)+ aspn23_lcm.type_kepler_orbit._get_hash_recursive(newparents)) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        if metadata_BeiDou_ephemeris._packed_fingerprint is None:
            metadata_BeiDou_ephemeris._packed_fingerprint = struct.pack(">Q", metadata_BeiDou_ephemeris._get_hash_recursive([]))
        return metadata_BeiDou_ephemeris._packed_fingerprint

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", metadata_BeiDou_ephemeris._get_packed_fingerprint())[0]
