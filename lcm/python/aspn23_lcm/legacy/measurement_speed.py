...
class measurement_speed(object):
    """ Speed is the magnitude of the velocity vector with respect to the user-specified reference frame. """

    __slots__ = ["icd_measurement_speed", "header", "time_of_validity", "reference", "speed", "variance", "error_model", "num_error_model_params", "error_model_params", "num_integrity", "integrity"]
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(measurement_speed._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack(">b", self.icd_measurement_speed))
        ...
        buf.write(struct.pack(">bddbi", self.reference, self.speed, self.variance, self.error_model, self.num_error_model_params))
    ...
    def decode(data: bytes):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != measurement_speed._get_packed_fingerprint():
            raise ValueError("Decode error")
        return measurement_speed._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        self = measurement_speed()
        self.icd_measurement_speed = struct.unpack(">b", buf.read(1))[0]
        self.header = aspn23_lcm.type_header._decode_one(buf)
        self.time_of_validity = aspn23_lcm.type_timestamp._decode_one(buf)
        self.reference, self.speed, self.variance, self.error_model, self.num_error_model_params = struct.unpack(">bddbi", buf.read(22))
    ...
    def _get_hash_recursive(parents):
        if measurement_speed in parents: return 0
        newparents = parents + [measurement_speed]
        tmphash = (0xdcd9871ed249b71a+ aspn23_lcm.type_header._get_hash_recursive(newparents)+ aspn23_lcm.type_timestamp._get_hash_recursive(newparents)+ aspn23_lcm.type_integrity._get_hash_recursive(newparents)) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        if measurement_speed._packed_fingerprint is None:
            measurement_speed._packed_fingerprint = struct.pack(">Q", measurement_speed._get_hash_recursive([]))
        return measurement_speed._packed_fingerprint

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", measurement_speed._get_packed_fingerprint())[0]
