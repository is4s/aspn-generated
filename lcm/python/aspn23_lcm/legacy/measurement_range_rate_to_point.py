...
class measurement_range_rate_to_point(object):
    """
    ...
    __slots__ = ["icd_measurement_range_rate_to_point", "header", "time_of_validity", "remote_point", "obs", "variance", "error_model", "num_error_model_params", "error_model_params", "num_integrity", "integrity"]

    __typenames__ = ["int8_t", "aspn23_lcm.type_header", "aspn23_lcm.type_timestamp", "aspn23_lcm.type_remote_point", "double", "double", "int8_t", "int32_t", "double", "int16_t", "aspn23_lcm.type_integrity"]
    ...
    def __init__(self):
        self.icd_measurement_range_rate_to_point = 0
        """
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(measurement_range_rate_to_point._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack(">b", self.icd_measurement_range_rate_to_point))
    ...
    def decode(data: bytes):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != measurement_range_rate_to_point._get_packed_fingerprint():
            raise ValueError("Decode error")
        return measurement_range_rate_to_point._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        self = measurement_range_rate_to_point()
        self.icd_measurement_range_rate_to_point = struct.unpack(">b", buf.read(1))[0]
    ...
    def _get_hash_recursive(parents):
        if measurement_range_rate_to_point in parents: return 0
        newparents = parents + [measurement_range_rate_to_point]
        tmphash = (0x8741333669c1487d+ aspn23_lcm.type_header._get_hash_recursive(newparents)+ aspn23_lcm.type_timestamp._get_hash_recursive(newparents)+ aspn23_lcm.type_remote_point._get_hash_recursive(newparents)+ aspn23_lcm.type_integrity._get_hash_recursive(newparents)) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        if measurement_range_rate_to_point._packed_fingerprint is None:
            measurement_range_rate_to_point._packed_fingerprint = struct.pack(">Q", measurement_range_rate_to_point._get_hash_recursive([]))
        return measurement_range_rate_to_point._packed_fingerprint

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", measurement_range_rate_to_point._get_packed_fingerprint())[0]
